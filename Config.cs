...
namespace OrkunDemos
{
        ...
        public class Config : INotifyPropertyChanged
        {
            ...
            public static string ConfigFile
            {
                get
                {
                    _configPath = _configPath ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "UITrackBarRuler");

                    if (!Directory.Exists(_configPath))
                        Directory.CreateDirectory(_configPath);

                    _configFile = _configFile ?? Path.Combine(_configPath, "UITrackBarRuler.json");
